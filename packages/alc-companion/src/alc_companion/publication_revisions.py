...
from ac_jobs import atomic_write_bytes, canonical_json_bytes
...
PUBLICATION_REVISION_REQUEST_SCHEMA = (
    "alc.companion.publication_revision_request.v1"
)
PUBLICATION_REVISION_RESULT_SCHEMA = (
    "alc.companion.publication_revision_result.v1"
)
PUBLICATION_REVISION_BUNDLE_SCHEMA = (
    "alc.companion.publication_revision_bundle.v1"
)
...
def commit_publication_revision(
    ...
    for replacement in request.replacements:
        ...
        provenance.update(
            {
                "last_editor": "alc-companion-review",
...
def _known_durable_revision_digests(
    ...
    known = {
        ...
        if item.revision == 1
        and item.provenance.get("producer") != "alc-render-browser"
    }
