...
class CompanionBuildHandler:
    """Coordinate alc-translate and reviewed Companion guide generation."""
    ...
    def _model_source_inputs(
        ...
        paper = AcDocumentService(cache_root=self.execution.document_cache_root)
        ...
        for chapter in chapters:
            ...
            cached_source = paper.repository.store_bytes(
                ...
                origin=SourceOrigin(
                    ...
                    locator=(
                        f"alc-companion:{context.run_id}:{view_artifact}"
                    ),
                ),
            )
            cached_document = cached_document_ref_to_document(
    ...
    def _authors(
        ...
        *,
        ...
        context.artifacts.publish_json(
            "diagnostics/author",
            {
                "schema_version": "alc.companion.author_diagnostics.v1",
                "status": (
                    "confirmed"
                    if value["confidence"] == "high"
                    else "unconfirmed"
    ...
    def _chapter_model_context(
        ...
        *,
        ...
        source_commands = _chapter_source_commands(
            context,
            chapter,
            access,
            cache_root=self.execution.document_cache_root,
        ...
        source_commands["translation"] = _chapter_translation_commands(
            chapter,
            translation_index,
            cache_root=self.execution.document_cache_root,
        )
...
def _chapter_source_commands(
    ...
    *,
    ...
    document = AcDocumentService(cache_root=cache_root)
...
def _companion_llm_options(
    ...
    cache_root = AcDocumentService(
        cache_root=execution.document_cache_root
    ).cache_root
    values["AC_DOCUMENT_CACHE"] = str(cache_root)
    path_value = values.get("PATH") or ""
    command = shutil.which("ac-document", path=path_value)
    if command is None:
        executable_name = (
