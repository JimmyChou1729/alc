...
from alc_companion.translation_adapter import (
    AlcTranslateAdapter,
...
class FakeTranslationAdapter:
    ...
    def build_glossary(self, _context, source, **kwargs):
        ...
        return {
            "schema_version": "alc.translate.glossary_result.v1",
            "document_digest": source.document_digest,
            "source_digest": source.source.artifact_digest,
            "target_language": kwargs["target_language"],
    ...
    def translate_blocks(self, context, source, **kwargs):
        ...
        return TranslationResult(
            ...
            coverage="selection",
            layer=Layer(
                source_identity, "alc-translate", tuple(revisions)
            ),
...
def test_translation_precedes_reviewed_guides_and_uses_local_glossary(
    ...
    for payload in guide_payloads:
        ...
        assert all(
            item["shell"]
            and item["argv"][0:2]
            == ["ac-document", "read-cached-source-range"]
    ...
    translated_range = AcDocumentService(
...
def test_translation_durable_units_freeze_the_lane_contract(
    ...
    assert all(
        ...
        == "alc.companion.translation_lane.v1"
...
def test_structural_display_chapter_skips_loop_but_translates_and_augments(
    ...
    document = _document(tmp_path)
    paper = AcDocumentService(cache_root=tmp_path / "paper")
...
def test_same_language_skips_all_translation_owned_steps(
    ...
    service = CompanionService(tmp_path / "jobs")
    monkeypatch.setattr(
        "alc_companion.service.require_translation_runtime",
...
def test_cached_document_parse_failure_is_not_downgraded(
    ...
    document = _document(tmp_path)
    monkeypatch.setattr(
        "alc_companion.build.AcDocumentService.cache_document",
...
def test_default_adapter_wires_keyword_provider_to_companion_cache(
    ...
    from alc_translate import TranslationWorkflowService
...
def test_default_adapter_preflight_requires_public_translate_facade(
    ...
    def blocked_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "alc_translate":
            raise ImportError("incomplete alc-translate facade")
    ...
    with pytest.raises(
        CompanionTranslationRuntimeError,
        match="complete compatible alc-translate runtime",
    ) as exc_info:
        require_translation_runtime()
...
@pytest.mark.parametrize(
    "handler",
    (
        "alc.companion.build.v6",
        "alc.companion.build.v10",
        "alc.companion.build.v11",
    ),
)
